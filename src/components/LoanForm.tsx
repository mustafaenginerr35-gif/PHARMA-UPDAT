...
export const LoanForm: React.FC<LoanFormProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === 'outgoing') {
      submissionData.expectedReturnDate = new Date(formData.expectedReturnDate);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <CurrencyInput
            ...
            onUpdate={(val) => setFormData({ ...formData, amount: val })}
                ...
                <SelectContent className="bg-card border-border">
                  {openLoans.map(loan => (
                    <SelectItem key={loan.id} value={loan.id!} className="font-bold">
                      {loan.partyName} - {loan.amount.toLocaleString()} د.ع ({format(new Date(loan.date), 'yyyy/MM/dd')})
                    </SelectItem>
                  ))}
                  {openLoans.length === 0 && (
